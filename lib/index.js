...
function SlackBot(events, plugin) {
	...
	this.rtm.on(this.RTM_EVENTS.MESSAGE, function(message) {
		...
		if (message.text && message.text.split(' ')[0] === identifier) {
			...
			if (self.pluginConfig.test || !self.getCommand()) {
				if (self.testCommand(message.text)) {
					if (self.testSyntax(message.text)) {
						events.emit('message', message);
					} else {
						self.send(self.getSyntaxMessage(), message.channel);
					}
				}
			} else {
				events.emit('message', message);
	...
	this.rtm.on(this.RTM_EVENTS.REACTION_ADDED, function(message) {
		events.emit('reaction_added', message);
	});
}
...
SlackBot.prototype.testCommand = function(message) {
	return this.pluginConfig.command === message.split(' ')[0].toLowerCase();
};
